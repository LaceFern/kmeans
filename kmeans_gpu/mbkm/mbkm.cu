...
__global__ void cal_dist_kst32(
    ...
    int index = 0;
    if((warp_id < sampleInblock * k) && (warp_id < (batch_size - block_idx * sampleInblock) * k)){
        int sample_id = d_random_index_array[block_idx * sampleInblock + warp_id / k];
        // int sample_id = block_idx * sampleInblock + warp_id / k + 1l * batch_id * batch_size;
        dist_array[thread_idx] = 0;
        int i;
        for(i = 0; i < ((d + 31)>>5) - 1; i++){
            coor_1 = dmatrix[1l * sample_id * d + i * 32 + lane_id];
            coor_2 = means[(warp_id % k) * d + i * 32 + lane_id];
            dist_array[thread_idx] += (coor_1 - coor_2) * (coor_1 - coor_2);
        }
        if(lane_id < d - 32 * i){
        ...
        if(warp_id < sampleInblock && warp_id < (batch_size - block_idx * sampleInblock)){
            ...
            sample_id = d_random_index_array[block_idx * sampleInblock + warp_id];
            // sample_id = 1l * batch_id * batch_size + block_idx * sampleInblock + warp_id;
            for(int j = 0; j < (d / 32 + 1); j++){
                if(lane_id < d - 32 * j){
                    float temp = dmatrix[1l * sample_id * d + j * 32 + lane_id];
...
__global__ void cal_dist_klt32(
    ...
    for(int iter = 0; iter < iterForcenter; iter++){
        ...
        if((warp_id < 32) && warp_id < (k - iter * 32)){
            //这个参数i是干什么的->是为了维度并行
            int sample_id = d_random_index_array[block_idx];
            // int sample_id = block_idx + 1l * batch_id * batch_size;
            int i = 0;
            for(i = 0; i < ((d + 31)>>5) - 1; i++){
                coor_1 = dmatrix[1l * sample_id * d + i * 32 + lane_id];
                coor_2 = means[(iter * 32 + warp_id) * d + i * 32 + lane_id];
...
// __global__ void gen_random_index_array(int* random_index_array, int batch_size, int n, int batch_id){
//     int idx = blockIdx.x * blockDim.x + threadIdx.x;
//     if(idx < batch_size){
//         thrust::minstd_rand engine;
//         engine.discard(idx);
//         thrust::uniform_int_distribution<> dist(0, n - 1);
//         random_index_array[idx] = dist(engine);   
//         // random_index_array[idx] = idx + batch_id * batch_size;
//     }
// }
...
//--------------------------------------------
        ...
        // dim3 gen_random_index_array_block((batch_size - 1) / 1024 + 1, 1);
        // dim3 gen_random_index_array_thread(1024, 1);
        // gen_random_index_array<<<gen_random_index_array_block, gen_random_index_array_thread, 0, stream>>>(d_random_index_array, batch_size, n, epoch_count);
        // cudaDeviceSynchronize();
        for (int j = 0; j < batch_size; j++) {
            h_random_index_array[j] = rand() % n;
            // if(i==0) printf("%d\t", h_random_index_array[j]);
        }
        // CHECK(cudaMemcpy(d_random_index_array, h_random_index_array, batch_size * sizeof(int), cudaMemcpyHostToDevice));
        cudaMemcpy(d_random_index_array, h_random_index_array, batch_size * sizeof(int), cudaMemcpyHostToDevice);
...
void start_mbkm(int n, int d, int k, char *filename, float *dmatrix, float *means, int batchsize, int seed, int threads){
    ...
    int* h_random_index_array = (int*) ddr_alloc(batchsize * sizeof (int));

    int* d_random_index_array;
    cudaMalloc(&d_random_index_array, batchsize * sizeof(int));
    ...
    fprintf(fp, "epoch_num: %d; n: %d; d: %d; k: %d; batchsize: %d; seed: %d\n", \
        epoch_num, n, d, k, batchsize, seed);
    ...
    // printf("loss:%lf\n",loss);
    free(accu_runtime);
    free(h_random_index_array);
    cudaFree(d_means);
    cudaFree(d_dmatrix);
    cudaFree(d_sum);
    cudaFree(d_len);
    cudaFree(d_random_index_array);
}
        ...
        for(int seed_idx = 0; seed_idx < SEED_NUM; seed_idx++){
            ...
            for(int k_idx = 0; k_idx < K_NUM; k_idx++){
                ...
                for(int bs_idx = 0; bs_idx < BS_NUM; bs_idx++){
                    ...
                    start_mbkm(n, d, k, dsinfo_arr[dsinfo_idx].filename, dmatrix, means, batchsize, seed, threads); //打印：只打印前10个
                }
                free(means);
            }
        }
